...
| 方法 | 签名 | 说明 |
|------|------|------|
| `list` | `(opts?) => Promise<SessionSummary[]>` | 列出会话。支持 `userId`、`limit`、`cursor` 分页。 |
| `get` | `(conversationId) => Promise<SessionSummary \| null>` | 查询单个会话摘要。 |
| `delete` | `(conversationId) => Promise<void>` | 删除会话记录。 |

```typescript
const summaries = await agent.sessions.list({ userId: 'user-1', limit: 20 })
const one = await agent.sessions.get(session.id)
