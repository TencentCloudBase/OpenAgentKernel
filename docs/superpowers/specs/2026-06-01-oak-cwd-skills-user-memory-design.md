...
interface AgentConfig {
   ...
   *
   * ⚠️ 前提条件(业务方需保证):同一 userId 的请求必须路由到**同一个 SDK 节点**
   * (固定路由 / 队列串行 / 上游互斥锁,任选其一)。SDK **不在多节点并发场景下做冲突
   * 防御** — 跨节点并发会导致后写覆盖前写,数据丢失。详见 §5.3。
   *
...
- **同一 userId 的请求路由到同一个 SDK 节点** — SDK 不在多节点并发场景下做冲突检测/合并。跨节点并发会导致**后写覆盖前写**,数据丢失。业务方实现路径任选:
  - 固定路由(consistent hashing by userId)
  - 队列串行(同 userId 的请求进同一个队列)
  - 上游互斥锁(Redis lock by userId)
- **如果业务方做不到固定路由**,V2 评估"多节点并发模式"(详见 §13.1),可选启用 ETag + 三方合并
...
| 跨节点并发安全 | DB 行级 / namespace 隔离天然安全 | **业务方上游保证路由**(SDK 不防御,V2 可选启用 ETag) |
...
MVP 假设业务方上游保证"同 user 路由固定节点"。V2 评估为客户提供"多节点并发安全"模式作为可选启用:
...
- **API**:`userMemory.concurrency?: 'single-node' | 'multi-node'`(默认 `'single-node'` = MVP 行为)
